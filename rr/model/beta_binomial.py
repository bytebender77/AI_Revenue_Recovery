...
class BetaBinomialModel:
    ...
    def fit(self, observations: Iterable[dict], seed: int = 4242) -> "BetaBinomialModel":
        ...
        counts: dict[int, dict[str, list[int]]] = {i: {} for i in range(len(LEVELS))}
        for r in rows:
    ...
    def lookup(self, **feat) -> CellPosterior:
        ...
        return fallback or CellPosterior(0.5, 0.0, 1.0, 0, 0, len(LEVELS), "(unfitted)")

    # ------------------------------------------------------------------- io --
    def save(self, path: pathlib.Path) -> None:
        path.write_text(json.dumps(
            {"levels": [list(l) for l in LEVELS], "config": asdict(self.cfg),
